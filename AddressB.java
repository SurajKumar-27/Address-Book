...
class AddressBook {
    ...
    void addperson(Boolean s, Boolean t, Boolean u, Boolean v) {
		if(!s && !t && !u && !v){
			...
			if(name.trim().length()!=0 && add.trim().length()!=0 && (name.split("\\s")[0].chars()).allMatch(x->Character.isLetter(x))){
...
class frame1 extends JFrame
{
	public frame1()
	{
		...
		sub.setForeground(Color.decode("#34495E"));
